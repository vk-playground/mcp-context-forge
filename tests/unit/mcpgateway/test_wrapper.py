# -*- coding: utf-8 -*-
"""Tests for the MCP *wrapper* module.
...
def _install_fake_mcp(monkeypatch) -> None:
    ...
    class _FakeServer:
        ...
        # decorators just hand the coro straight back
        def list_tools(self):      return lambda fn: fn        # noqa: E704
        def call_tool(self):       return lambda fn: fn        # noqa: E704
        def list_resources(self):  return lambda fn: fn        # noqa: E704
        def read_resource(self):   return lambda fn: fn        # noqa: E704
        def list_prompts(self):    return lambda fn: fn        # noqa: E704
        def get_prompt(self):      return lambda fn: fn        # noqa: E704
    ...
    # ── stdio helper used inside wrapper.main() ──────────────────────────── #
    ...
    # ── pydantic model placeholder that *accepts* kwargs ────────────────── #
    class _InitOpts:
    ...
    # ── ultra-thin DTOs used by the wrapper's handlers ───────────────────── #
    class _Tool:
...
def wrapper(monkeypatch):
    ...
    sys.modules.pop("mcpgateway.wrapper", None)                 # fresh import
...
@pytest.mark.parametrize(
    ("raw", "expected"),
    [
        # path *not* ending in "servers" ⇒ path is preserved
        ("https://x.com/s/1",                 "https://x.com/s/1"),
...
async def test_fetch_url_ok(monkeypatch, wrapper):
    class _Client:
        ...
        async def get(self,url,**_): _Client.url=url; return _Resp(json_data={"ok":1})
...
async def test_fetch_url_http_status(monkeypatch, wrapper):
    class _Client:
        ...
        async def __aenter__(self): return self
        async def __aexit__(self,*_): return False
        async def get(self,*_a,**_k): return _Resp(status=500)
...
# -- helper: patch AsyncClient for POST calls -------------------------------- #
...
async def test_read_resource(monkeypatch, wrapper):
    async def _fake(u): return _Resp(text="body")
...
async def test_get_prompt(monkeypatch, wrapper):
    async def _fake(_): return _Resp(json_data={"template":"Hi {n}","description":""})
    monkeypatch.setattr(wrapper,"fetch_url",_fake)
...
async def test_get_prompt_missing(monkeypatch, wrapper):
    async def _fake(_): return _Resp(json_data={"template":"Hi {n}"})
    monkeypatch.setattr(wrapper,"fetch_url",_fake)
...
async def test_handle_list_tools(monkeypatch, wrapper):
    async def _ids(_): return ["1"]
    async def _meta(_):return [{"name":"A","description":"","inputSchema":{}}]
    monkeypatch.setattr(wrapper,"get_tools_from_mcp_server",_ids)
    monkeypatch.setattr(wrapper,"tools_metadata",_meta)
    t = await wrapper.handle_list_tools()
...
def test_main_runs_ok(wrapper):
    # reset & run
    wrapper.server.__class__.was_run = False
